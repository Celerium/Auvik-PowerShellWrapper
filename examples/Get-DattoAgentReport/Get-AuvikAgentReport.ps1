<#
    .SYNOPSIS
        Gets a BCDR agent report from the Auvik API.

    .DESCRIPTION
        The Get-AuvikAgentReport script gets a BCDR agent report from the Auvik API.

        Can be run against a single BCDR device or all BCDR devices registered in the Auvik portal

        This is a proof of concept script. It is not intended to be used in production.

    .PARAMETER Api_Key_Public
        Defines your Auvik API public key.

    .PARAMETER Api_Key_Secret
        Defines your Auvik API secret key.

    .PARAMETER APIEndPoint
        Define what Auvik endpoint to connect to.

        The default is https://auvikapi.us1.my.auvik.com/v1
    ...
    .EXAMPLE
        Get-AuvikAgentReport -Api_UserName 12345 -Api_Key 12345

        Gets all BCDR agents from every registered BCDR in the Auvik portal.

        Returned data is outputted to a CSV file.

    .EXAMPLE
        Get-AuvikAgentReport -Api_UserName 12345 -Api_Key 12345 -serialNumber 1234567890
    ...
    .EXAMPLE
        Get-AuvikAgentReport -Api_UserName 12345 -Api_Key 12345 -Report All
    ...
    .LINK
        https://github.com/Celerium/Auvik-PowerShellWrapper
...
#Region    [ Parameters ]
    ...
    param(
        ...
        $APIEndpoint = 'https://auvikapi.us1.my.auvik.com/v1',
...
#Region    [ Prerequisites ]
    ...
    $ScriptName = 'Get-AuvikAgentReport'
    ...
    Try {
        ...
        If(Get-Module -ListAvailable -Name AuvikAPI) {
            Import-module AuvikAPI -ErrorAction Stop
        }
        Else {
            Install-Module AuvikAPI -Force -ErrorAction Stop
            Import-Module AuvikAPI -ErrorAction Stop
        }
    }
    Catch {
    ...
    #Settings Auvik login information
    Add-AuvikBaseURI -base_uri $APIEndpoint
    Add-AuvikAPIKey -Api_UserName $Api_UserName -Api_Key $Api_Key -ErrorAction Stop
...
Write-Output " - (1/3) - $(Get-Date -Format MM-dd-HH:mm) - Getting Auvik agents from the API"
...
#Region     [ Auvik API ]
    ...
    Try{
        ...
        if ($serialNumber){
            ...
            $AuvikBCDR = Get-AuvikDevice -serialNumber $serialNumber
            $AuvikAgents = (Get-AuvikAgent -serialNumber $serialNumber).items

            $AuvikAgentResults = $AuvikAgents | Select-Object `
                @{Name='resellerCompanyName';Expression={$AuvikBCDR.resellerCompanyName}}, `
                @{Name='clientCompanyName';Expression={$AuvikBCDR.clientCompanyName}}, `
                @{Name='BCDR';Expression={$AuvikBCDR.Name}}, `
                @{Name='BCDRip';Expression={$AuvikBCDR.internalIP}}, `
                @{Name='Model';Expression={$AuvikBCDR.Model}}, `
                @{Name='serialNumber';Expression={$AuvikBCDR.serialNumber}}, `
            ...
            $AuvikBCDR = (Get-AuvikDevice).items
            $AuvikAgentResults = ForEach ($BCDR in $AuvikBCDR){

                $Agents = (Get-AuvikAgent -serialNumber $BCDR.serialNumber).items
...
#Region     [ CSV Report ]
    Try{
        If($Report -eq 'All' -or $Report -eq 'CSV'){
            ...
            $AuvikAgentResults | Sort-Object clientCompanyName | Select-Object $ScriptName,* | Export-Csv $CSVReport -NoTypeInformation
        }
    }
    Catch{
        Write-Host $_.Exception.Message -ForegroundColor Red -BackgroundColor Black
...
#Region    [ HTML Report]
    ...
    Try{
        If ($Report -eq 'All' -or $Report -eq 'HTML'){
            ...
            $TotalAgents                = ($AuvikBCDR.agentCount | Measure-Object -Sum).sum
            $AgentVersion               = ($AuvikAgentResults.AgentVersion | Select-Object -Unique).Count
            $TotalProtectedVolumes      = ($AuvikAgentResults.protectedVolumesCount | Measure-Object -Sum).sum
            $TotalUnProtectedVolumes    = ($AuvikAgentResults.unprotectedVolumesCount | Measure-Object -Sum).sum
            $TotalPaused                = ($AuvikAgentResults | Where-Object {$_.isPaused -eq 'True'}).count
            $TotalArchived              = ($AuvikAgentResults | Where-Object {$_.isArchived -eq 'True'}).count
    ...
    #Region    [ Example HTML Report Data\Structure ]
        ...
        $TableResults = $AuvikAgentResults | ConvertTo-Html -As Table -Fragment -Property clientCompanyName,Agent,AgentVersion,protectedVolumeNames,unprotectedVolumeNames,isPaused,isArchived `
                                        -PostContent    '   <ul>
