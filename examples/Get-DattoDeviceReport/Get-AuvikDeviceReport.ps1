<#
    .SYNOPSIS
        Gets an BCDR device report from the Auvik API.

    .DESCRIPTION
        The Get-AuvikDeviceReport script gets an BCDR device report from the Auvik API.
    ...
    .EXAMPLE
        Get-AuvikDeviceReport -Api_UserName 12345 -Api_Key 12345

        Gets all registered BCDR's from the Auvik portal.

        Returned data is outputted to a CSV file.

    .EXAMPLE
        Get-AuvikDeviceReport -Api_UserName 12345 -Api_Key 12345 -serialNumber 1234567890

        Gets the defined BCDR device from the Auvik portal.

        Returned data is outputted to a CSV file.

    .EXAMPLE
        Get-AuvikDeviceReport -Api_UserName 12345 -Api_Key 12345 -Report All
...
#Region    [ Prerequisites ]
    ...
    $ScriptName = 'Get-AuvikDeviceReport'
...
Write-Output " - (1/3) - $(Get-Date -Format MM-dd-HH:mm) - Getting Auvik BCDR's from the API"
...
#Region     [ Auvik API ]
    ...
    Try{
        ...
        if ($serialNumber){
            $AuvikDeviceResults = Get-AuvikDevice -serialNumber $serialNumber
        }
        else{
            $AuvikDeviceResults = Get-AuvikDevice
            $AuvikDeviceResults = ($AuvikDeviceResults).items
        }

        $AuvikAgentResults = $AuvikDeviceResults | Select-Object `
...
#Region    [ HTML Report]
    ...
    Try{
        If ($Report -eq 'All' -or $Report -eq 'HTML'){
            ...
            $TotalDevices   = ($AuvikAgentResults.serialNumber).count
            $DeviceTypes    = ($AuvikAgentResults.Model | Select-Object -Unique).count
            $LocalStorage   = $(Convert-BytesToSize -Size $( ($AuvikDeviceResults.localStorageUsed | Measure-Object -Property Size -Sum).sum * 1024))
            $OffsiteStorage = $(Convert-BytesToSize -Size $( ($AuvikDeviceResults.offsiteStorageUsed | Measure-Object -Property Size -Sum).sum ))
            $ActiveTickets  = ($AuvikAgentResults.activeTickets | Measure-Object -Sum).sum
            $TotalAgents    = ($AuvikAgentResults.agentCount | Measure-Object -Sum).sum
            $TotalShares    = ($AuvikAgentResults.shareCount | Measure-Object -Sum).sum
            $TotalAlerts    = ($AuvikAgentResults.alertCount | Measure-Object -Sum).sum
    ...
    #Region    [ Example HTML Report Data\Structure ]
        ...
        $TableResults = $AuvikAgentResults | ConvertTo-Html -As Table -Fragment -Property clientCompanyName,status,Name,Model,localStorageUsed,offsiteStorageUsed,lastSeenDate,warrantyExpire `
                                        -PostContent    '   <ul>
