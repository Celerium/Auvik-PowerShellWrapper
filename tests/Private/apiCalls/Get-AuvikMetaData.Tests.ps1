<#
    .SYNOPSIS
        Pester tests for the AuvikAPI PLACEHOLDER functions
...
#Can be used in [ It ] with [ -TestCases @{ VariableName = $VariableName } ]
    BeforeDiscovery{
        ...
        #$withoutAuth = $( [bool]$Api_UserName -eq $false -or [bool]$Api_Key -eq $false )
...
Describe "Testing [ $commandName ] function with [ $pester_TestName ]" -Tag @('PLACEHOLDER','apiCalls') {
    ...
    Context "[ $commandName ] testing function" {
        ...
        It "With a good API key should authenticate" -Skip:$withoutAuth {
            Add-AuvikBaseUri
            Add-AuvikAPIKey -Api_UserName $Api_UserName -Api_Key $Api_Key
