...
Describe "Testing [ $commandName ] function with [ $pester_TestName ]" -Tag @('apiKeys') {
    ...
    Context "[ $commandName ] testing function" {
        ...
        It "Running [ $commandName ] should remove all apiKey variables" {
            ...
            Remove-AuvikAPIKey
            $auvik_UserName | Should -BeNullOrEmpty
            $auvik_ApiKey | Should -BeNullOrEmpty
        }

        It "If the [ auvik_UserName ] is already empty a warning should be thrown" {
            Add-AuvikAPIKey -Api_UserName '12345' -Api_Key "AuvikApiKey"
            Remove-Variable -Name "auvik_UserName" -Scope global -Force

            Remove-AuvikAPIKey -WarningAction SilentlyContinue -WarningVariable apiKeyWarning
            $apiKeyWarning | Should -Be "The Auvik API [ public ] key is not set. Nothing to remove"
        }

        It "If the [ auvik_ApiKey ] is already empty a warning should be thrown" {
            Add-AuvikAPIKey -Api_UserName '12345' -Api_Key "AuvikApiKey"
            Remove-Variable -Name "auvik_ApiKey" -Scope global -Force

            Remove-AuvikAPIKey -WarningAction SilentlyContinue -WarningVariable apiKeyWarning
            $apiKeyWarning | Should -Be "The Auvik API [ secret ] key is not set. Nothing to remove"
        }
