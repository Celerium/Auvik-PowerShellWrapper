<#
    ...
    .EXAMPLE
        Invoke-Pester -Path .\Tests\Private\ModuleSettings\Get-AuvikModuleSettings.Tests.ps1

        Runs a pester test and outputs simple results

    .EXAMPLE
        Invoke-Pester -Path .\Tests\Private\ModuleSettings\Get-AuvikModuleSettings.Tests.ps1 -Output Detailed
...
Describe "Testing [ $commandName ] function with [ $pester_TestName ]" -Tag @('moduleSettings') {
    ...
    Context "[ $commandName ] testing function" {
        ...
        It "No configuration should populate baseline variables" {
            Import-AuvikModuleSettings -AuvikConfPath $invalidPath -AuvikConfFile 'invalid.psd1'

            (Get-Variable -Name Auvik_Base_URI).Value | Should -Be $(Get-AuvikBaseURI)
            (Get-Variable -Name Auvik_JSON_Conversion_Depth).Value | Should -Not -BeNullOrEmpty
        }
        ...
        It "Saved configuration session should contain required variables" {
            ...
            Import-AuvikModuleSettings -AuvikConfPath $exportPath

            (Get-Variable -Name Auvik_Base_URI).Value | Should -Not -BeNullOrEmpty
            (Get-Variable -Name auvik_UserName).Value | Should -Not -BeNullOrEmpty
            (Get-Variable -Name auvik_ApiKey).Value | Should -Not -BeNullOrEmpty
            (Get-Variable -Name Auvik_JSON_Conversion_Depth).Value | Should -Not -BeNullOrEmpty
        }
